...
import { AlertDialog, Card, Flex } from '@chialite/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hexsheck = /[\da-f]+$/gi;

    if (!hexsheck.test(ip_val) || ip_val.value === '') {
    ...
    const chialiteper_input = ip_parsed.limit;
    ...
    const chialiteper_value = Number.parseInt(Number(chialiteper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (chialite per interval):{' '}
                  {mojo_to_chialite_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
