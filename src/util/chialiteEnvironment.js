...
const PY_FOLDER = '../chialite/daemon';
...
const getChialiteVersion = () => {
  let version = null;
  const exePath = getExecutablePath('chialite');
  // first see if we can get a chialite exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or chialite is on the path
    try {
      version = child_process
